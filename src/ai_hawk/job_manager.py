...
class AIHawkJobManager:
        ...
        for position, location in searches:
            ...
            try:
                while True:
                    ...
                    should_continue = self.read_jobs()
    ...
    def get_jobs_from_page(self, scroll=False):
        ...
        try:   
            ...
            jobs_xpath_query = "(//div[contains(@class,'scaffold-layout__list')])[3]/div";
            jobs_container = self.driver.find_element(By.XPATH, jobs_xpath_query)
            
            if scroll:
                logger.warning(f'is scrollable: {browser_utils.is_scrollable(jobs_container)}')
            ...
            job_element_list = self.driver.find_elements(By.XPATH, "//li[contains(@class, 'scaffold-layout__list-item') and contains(@class, 'ember-view')]")
    ...
    def read_jobs(self):
        ...
        job_element_list = self.get_jobs_from_page(True)
        job_list = [self.job_title_to_job_collect(job_element) for job_element in job_element_list]         
        if not job_list:
            return False
        for job in job_list:            
            ...
            try:
                ...
                if not self.already_collected_job(job.link):
                    self.write_to_file(job,current_date.strftime("%Y-%m-%d") +'_data')
                else:
                    print ('skipping this job, already collected: ' + job.link)
            except Exception as e:
    ...
    def apply_jobs(self):
        ...
        job_list = [self.job_title_to_job(job_element) for job_element in job_element_list]
    ...
    def job_title_to_job(self, job_tile) -> Job:
        logger.debug("Extracting job information from tile")
        job = Job()

        try:            
            title = job_tile.find_element(By.XPATH, "//a[contains(@class,'job-card-list__title')]")
            job.title = title.find_element(By.TAG_NAME, 'strong').text
            logger.debug(f"Job title extracted: {job.title}")
        except NoSuchElementException:
            logger.warning("Job title is missing.")
        
        try:
            job.link = job_tile.find_element(By.XPATH, "//a[contains(@class,'job-card-list__title')]").get_attribute('href').split('?')[0]
    ...
    def job_title_to_job_collect(self, job_tile) -> Job:
        logger.debug("Extracting job information from tile")
        job = Job()
    ...
    def already_collected_job(self, link):
        file_name = "data"
        file_path = self.output_file_directory / f"{file_name}.json"
